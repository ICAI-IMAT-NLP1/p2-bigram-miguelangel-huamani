...
def bigrams_count_to_probabilities(
    ...
    for i, fila in enumerate(smoothed_counts):
        suma_fila = fila.sum()
        for j, count in enumerate(fila):
            if suma_fila == 0:
                smoothed_counts[i,j] = 0.0
            else:
                smoothed_counts[i,j] = (count/suma_fila).item()
...
def calculate_neg_mean_log_likelihood(
    ...
    # TODO
    for word in words:
        lh = calculate_log_likelihood(word, bigram_probabilities, char_to_index, start_token, end_token)
        total_log_likelihood += lh
        

    # Calculate and return the negative mean log likelihood
    # TODO
    mean_log_likelihood: float = (total_log_likelihood/len(words)).item()
...
def generate_name(
    ...
    while len(generated_name)<max_length and not end: 
...
def calculate_log_likelihood(
    ...
    for i in range(len(processed_word)-1):
        a_idx = char_to_index[processed_word[i]]
        b_idx = char_to_index[processed_word[i+1]]
        log_lh_ab = torch.log(bigram_probabilities[a_idx,b_idx])
        log_likelihood += log_lh_ab
